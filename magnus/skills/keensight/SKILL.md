...
1. **定标**——从触发语 + 追问锁定「前沿终点」与用户真实目标，并把终点锚成**可检验到达判据**（读懂某标志性论文 / 复现某关键结果）。
...
- **不凭印象 + 反幻觉**——一切陈述调工具核查、再三阅读；善用 parallel / loop sub-agent 调研并蒸馏，把宽度留在 sub-agent，别灌进主 context。前沿的关键定理 / 数据 / 引用必须查实（原始来源 / 跑码验算），无法核查者显式标注存疑。
